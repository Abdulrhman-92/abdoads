...
class WPIE_Images extends \wpie\import\base\WPIE_Import_Base {
        ...
        private function is_valid_image( $file = "" ) {
                ...
                if ( ! preg_match( '/[^\?]+\.(jpe?g|jpe|gif|png|bmp|tif|tiff|ico)\b/i', strtolower( trim( $file ) ) ) ) {
                        return false;
                }
                ...
                foreach ( $tag_images as $img_url ) {
                        ...
                        if ( $full_size_images ) {
                                ...
                                if ( $full_size != $img_url ) {
                                        ...
                                        if ( $image_headers !== false && isset( $image_headers[ 'Content-Type' ] ) && ! empty( $image_headers[ 'Content-Type' ] ) && strpos( $image_headers[ 'Content-Type' ], 'image' ) !== false ) {
                                                $img_url = $full_size;
                                        }
